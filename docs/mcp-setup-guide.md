...
- **예외**: `devoks-browser`의 `chrome-devtools-attach`(`:9269` 연결 전용 고유 설정)만 플러그인이 번들한다. `devoks-rn`의 `metro-devtools`는 Metro WebSocket URL이 재시작마다 바뀌므로 플러그인 번들·`claude mcp add`로 정적 등록할 수 없고, `~/.claude.json`에 `--wsEndpoint`를 수동 등록한다.
...
| **devoks-rn** (`metro-devtools-attach`) | `devoks-rn` 플러그인, Metro 실행 중 | `metro-devtools` in `~/.claude.json` (`--wsEndpoint`, Metro 재시작마다 갱신); `adb` / `xcrun simctl` (스크린샷) |
...
### 8-2. `~/.claude.json`에 MCP 서버 추가 (JS 상태 조회 시 필수)

Metro WebSocket device ID는 재시작마다 바뀌므로, 현재 URL을 추출해 직접 등록해야 합니다.
...
> ⚠ **주의**: 플러그인이 번들하는 `--browserUrl http://127.0.0.1:8081` 방식은 Metro CDP 응답 포맷 차이로 연결이 실패할 수 있습니다. 위 `--wsEndpoint` 수동 등록이 안정적입니다. Metro 재시작 후 URL을 재추출·갱신하고 Claude Code를 재시작하세요.
...
# 최소 구성 +
/plugin install devoks-rn@devoks-plugins
# Metro 실행 후 ~/.claude.json에 metro-devtools (--wsEndpoint) 추가 — 7절 참조
```
...
# RN 프로젝트: metro-devtools (--wsEndpoint) 추가 — 7절 참조
```
