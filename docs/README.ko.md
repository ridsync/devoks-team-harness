...
DevOks 팀 Claude Code 하네스 — 보안 엔지니어링·증거 기반 코드리뷰·기능개발·Git 워크플로우·React Native 디버깅 플러그인 모음.
...
| 플러그인 | 내용 | 필수 여부 |
|---------|------|----------|
| `devoks-core` | 기본 원칙, 보안 엔지니어링 기준, 컨벤션 preset, 명시적 setup 흐름 — MCP 점검 + 프로젝트 convention setup/management 제공 | **필수** |
| `devoks-git` | Git 커밋·이슈·PR 워크플로우 커맨드 | 권장 |
| `devoks-sdlc` | SDLC 워크플로우 통합 — 기능개발, 테스트 작성/triage, 증거 기반 코드리뷰, baseline/targeted 보안검증, 리팩토링, 요구사항·데이터흐름 검증 | 권장 |
...
# 2. 플러그인 설치
/plugin install devoks-core@devoks-plugins      # 필수 — core rule, 보안 기준, convention preset, setup 흐름
...
| **refs** | `security-engineering.md`를 포함한 `plugins/devoks-core/refs/*.md` | explicit setup 시 복사 |
...
SessionStart 훅은 이제 보안 기준 ref를 포함한 MCP/프로젝트 초기화 상태만 점검합니다.
...
| `/devoks-sdlc:code-review-general` | 보안 delta 분류를 포함한 범위 지정 증거 기반 코드리뷰 |
| `/devoks-sdlc:code-review-diff-branch` | 고위험 보안 승격을 포함한 브랜치 diff 리뷰 |
| `/devoks-sdlc:code-security-review` | baseline/targeted 서버·브라우저·의존성·위협 보안 검증 |
...
│   │   └── refs/                      # security-engineering 포함, setup 시 주입하는 reference docs
│   ├── devoks-git/commands/           # Git 커맨드 (3개)
│   ├── devoks-sdlc/                    # SDLC: 기능·테스트·코드·검증 (커맨드 8개 + 스킬 10개 + 에이전트 4개)
...
> `plugins/devoks-core/rules/agent-principles.md`, `memory-policy.md` 와 ASVS/OWASP 기준의 `security-engineering.md`를 포함한 `plugins/devoks-core/refs/*`는 explicit setup으로 프로젝트에 seed 됩니다. stack-specific `project-convention.md`는 `shared/conventions/*` preset에서 선택 주입되며, 이후에는 프로젝트가 소유합니다.
