...
| `devoks-feature` | 기능개발 워크플로우 (FRD·PLAN·실행 스킬, UI 구현) | 권장 |
| `devoks-verify` | 요구사항 체크리스트·구현 충실도 검증 + 데이터 흐름 정합성 검증 | 권장 |
| `devoks-code` | 코드리뷰·리팩토링·모듈분석·보안 검증 | 권장 |
| `devoks-test` | 자동화 테스트 작성 + 테스트 실행·실패 triage | 권장 |
...
# /plugin install devoks-rn@devoks-plugins          # 선택 — React Native 프로젝트
```
...
/plugin install devoks-git@devoks-plugins            # Git 워크플로우
/plugin install devoks-feature@devoks-plugins        # 기능개발
/plugin install devoks-verify@devoks-plugins         # 요구사항·데이터 흐름 검증
/plugin install devoks-code@devoks-plugins           # 코드 품질·보안 검증
/plugin install devoks-test@devoks-plugins           # 테스트 작성·triage
/plugin install devoks-browser@devoks-plugins        # 브라우저 도구 (선택)
/plugin install devoks-rn@devoks-plugins               # React Native 디버깅 (선택)
```
...
| `code-security-review` | `/devoks-code:code-security-review` *(에이전트 내부 전용)* | devoks-code |
...
| `code-security-reviewer` | devoks-code | `code-security-review` 커맨드 |
...
| `/devoks-code:code-security-review` | repo·의존성·위협 수준 보안 검증 |
...
│   ├── devoks-feature/                # 기능개발 (커맨드 3개 + 스킬 4개)
│   ├── devoks-verify/                 # 검증 (스킬 2개)
│   ├── devoks-code/                   # 코드 품질 (커맨드 5개 + 스킬 2개 + 에이전트 2개)
│   ├── devoks-test/                   # 테스트 (스킬 2개)
...
플러그인 구조·내용 개선 후보는 [`roadmap.md`](roadmap.md)에 TODO 형식으로 추적합니다. 주요 항목: `browser-visual-diff` → `devoks-verify:verify-visual-diff` 이전, Deploy/Release 단계용 `devoks-release` 신설, 카탈로그 표 자동 생성(README drift 방지), 심각도 분류 SSOT 통합.
