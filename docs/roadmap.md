...
| 우선순위 | 항목 |
|---|---|
| **높음** | 컨벤션 문서 계층 분리 · 스킬 평가/검증 워크플로우 보강 · UI 작업 분리 검토 · **`devoks-sdlc-orchestration` 신설(비-HITL SDLC)** |
| **중** | `devoks-release` 신설 · 구현자 세션 지속(continuation) · `browser-visual-diff` 이전 · 카탈로그 자동 생성 · evals 하니스 확대 · 심각도 분류 SSOT 통합 |
| **낮음** | 진입점 일관화 · Operate/Maintain 단계 · plugin 버전 메타 정합 |

---

## 완료 / 최근 반영

- [x] **보안 엔지니어링 기준 + React Web 보안 profile + 증거 기반 보안리뷰** (2026-07-13, `devoks-core` `1.1.3 → 1.2.0`, `devoks-sdlc` `1.3.0 → 1.4.0`)
  - `plugins/devoks-core/refs/security-engineering.md`를 ASVS 5.0.0·OWASP Cheat Sheets/WSTG·SAMM·NIST SSDF 기반 범용 SSOT로 추가하고 explicit setup과 SessionStart 준비도 점검에 연결.
  - `shared/conventions/react-web/project-convention.md`에 server authority, raw HTML/URL, cookie/session/CSRF, CSP, browser storage/cache, third-party script와 `Security Decisions`를 추가.
  - `code-review`는 필수 5축과 고위험 security delta 승격을, `code-security-review`는 baseline/targeted 모드·8개 점검 영역·source-to-sink·risk/confidence·not-run 계약을 적용.
  - `code-security-review/references/security-review-checklist.md`와 React 오탐·token storage·client-only auth·CSRF·server validation·not-run 등 eval 8개 추가.
...
- [ ] **컨벤션 preset 확장 / management 고도화**
  - 현재는 `react-web`·`react-native`·`android`·`ios` preset과 explicit setup/apply 모델까지 반영됨. 다음 단계는 preset 품질 고도화와 추가 stack 지원.
  - 후보: `nextjs`, `node-backend`, `fastapi`, `spring`, `swiftui-only`, `compose-only` 등 세분 preset 추가.
  - `project-convention-manage`에 provenance diff, selective apply, stale preset 감지, `CLAUDE.md` drift 정리까지 붙여 management UX를 강화한다.
  - 우선순위: 중(기반은 완료, 활용도 확장 단계).
...
- [x] **코드 리뷰 워크플로우 개선** (2026-07-13, `devoks-sdlc` `1.3.0 → 1.4.0`)
  ...
  - 반영: 필수 5축 개별 상태·근거, finding의 증거/영향·재현/검증·Confidence·후속 테스트, 보안 delta 승격 규칙.
...
- [ ] **evals 하니스 확대**
  - 현재 `feature-workflow-runner`와 `code-security-review`가 `evals/evals.json` 보유. `test-author`·`test-run-triage`·`verify-requirements`·`verify-data-flow`에도 추가 필요.
  - eval 러너 스크립트가 없음(데이터만 존재) → 공용 러너 도입 검토.
  - 우선순위: 중.

- [ ] **심각도 분류 SSOT 통합**
  - `code-review`·`code-security-review`는 `.claude/refs/code-review.md §8`의 Likelihood × Impact 기준으로 수렴 완료. `verify-requirements`·`test-run-triage`의 중복 정의 정리 필요.
  - `.claude/refs/code-review.md §8` 단일 참조로 수렴하고 각 스킬은 링크만.
  - 우선순위: 중(DRY/SSOT 위반 소지).

- [x] **`code-security-review` references/ 보강** (2026-07-13)
  - 서버·브라우저·인증·데이터·공급망·배포·운영 카테고리와 자동화/오탐 판정 원칙을 `references/security-review-checklist.md`로 분리.
