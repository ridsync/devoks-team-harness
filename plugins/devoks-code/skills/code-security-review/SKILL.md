...
**경계 (중복 금지):** `devoks-code:code-review`에도 보안 점검(🔒 보안 점검 요약)이 있으나, 그것은 *이번 변경분(diff)에 대한 인라인 경량 스크리닝*이다. 이 스킬은 *변경 여부와 무관하게 repo 전수 + 의존성 + 위협 모델링*을 본다.
...
- 일반 코드 품질 리뷰 → `devoks-code:code-review`.
- 라이선스 감사.
...
- 관련: `devoks-code:code-review`(변경분 인라인 보안 스크리닝 — 본 스킬과 역할 분리).
