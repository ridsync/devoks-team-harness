---
description: Figma 디자인과 라이브 렌더를 비교해 UI 충실도를 끌어올린다. Phase 0(슬롯 합의·Chrome 연결 확인) → Phase 1~4를 browser-visual-diff-capture 에이전트에 위임(캡처·시각diff·디자인가이드 정합성 검증 결과를 충실도 리포트로 수령) → Phase 5(미세조정) → Phase 6(회수+검증). Figma 비교 / 시각 diff / 픽셀 퍼펙트 / UI 충실도 / Figma vs 실제 렌더 키워드에서 호출.
metadata:
  author: ridsync
  version: 1.0.0
---
...
| Chrome DevTools MCP (`mcp__chrome-devtools-attach__*`) | 라이브 렌더 캡처 (**기본 도구**) | **필수** |
...
> ⚠️ **실행 주체는 `browser-visual-diff-capture` 에이전트이다.**  
> 스크린샷·멀티모달 바이트는 에이전트 컨텍스트에 격리되고, 메인 루프에는 충실도 리포트만 리턴된다.

Phase 0에서 확정된 슬롯 값으로 `browser-visual-diff-capture` 에이전트를 호출한다:
...
| 파일 | 내용 | 참조 주체 |
|------|------|---------|
| `references/figma-capture.md` | Figma MCP 3개 도구 사용 가이드 + URL 파싱 + 스케일 변환 | browser-visual-diff-capture 에이전트 |
| `references/live-capture.md` | Playwright/Chrome DevTools MCP 캡처 가이드 + 충돌 해결 | browser-visual-diff-capture 에이전트 |
| `references/diff-checklist.md` | 화면 카테고리별 시각 diff 체크리스트 템플릿 | browser-visual-diff-capture 에이전트 |
| `references/state-patch-pattern.md` | 임시 state 강제 진입 패턴 + git restore 회수 | browser-visual-diff-capture 에이전트 |
| `references/design-guide-mapping.md` | 프로젝트 디자인 가이드 ↔ Figma 메타데이터 매핑 가이드 | browser-visual-diff-capture 에이전트 |
