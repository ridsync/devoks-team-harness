...
- **한다:** 구현 착수 전 브랜치·이슈 사전체크(제안→확인→적용), 구현, 관련 테스트/린트 실행, PLAN 체크박스·status 갱신, 진행 추적, 마무리 보고. → `../feature-workflow-runner/references/branch-issue-precheck.md`
...
6. **진행 갱신** — **검증 통과 후에만** PLAN `- [ ]` → `- [x]`, 첫 착수 시 frontmatter `status: approved → in-progress`. 실패·미완이면 `[x]`로 바꾸지 않고 막힌 이유 명시.
