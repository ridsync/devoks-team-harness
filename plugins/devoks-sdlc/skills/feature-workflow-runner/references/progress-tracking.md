...
5. **손익분기 재확인** — 잔여 미완 Task 수를 다시 센다. PLAN 작성 시점엔 장기였더라도 재개 시점에 잔여가 3개 이하로 줄었다면, 남은 `size: M` Task도 이후 실행에서는 직접 구현을 우선 검토한다(마커 자체는 고치지 않고, 라우팅 판단만 현재 잔여량 기준으로 재적용). → `task-delegation.md` "위임 손익분기"
