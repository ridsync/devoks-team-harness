...
> 저장한 뷰 이름은 나중에 바꾸거나 필요 없으면 삭제할 수 있으면 좋겠고요.
...
REQ-003: 저장 뷰 관리(이름 변경·삭제)
- `AC-003-1` WHEN 사용자가 저장된 뷰의 이름을 변경 THE SYSTEM SHALL 변경된 이름으로 뷰 목록을 갱신한다.
- `AC-003-2` WHEN 사용자가 저장된 뷰를 삭제 THE SYSTEM SHALL 뷰 목록에서 제거한다.

### 4. Design Spec (발췌)

**4.2 컴포넌트 구조**
- `SavedViews` — 뷰 목록·저장·선택·이름변경·삭제 UI. 뷰 목록 상태를 소유.
  - 저장 버튼 / 뷰 선택 드롭다운 / 이름변경·삭제 메뉴 / 미존재 키 경고 토스트(조건부 렌더링)

**4.3 적용 패턴 (DSN)**
| ID | 설계 결정 | 근거 (프로젝트 패턴/제약) |
|----|-----------|---------------------------|
| DSN-001 | 뷰 저장/조회/수정/삭제는 기존 저장소 훅 패턴을 재사용 | 상태관리 SSOT |
...
### 1. Approach
- **요약:** 필터 직렬화/역직렬화 유틸 + 뷰 저장소 훅으로 저장·적용·이름변경·삭제(모두 로컬)를 구현하고, 공유(외부 API 연동)만 별도 흐름으로 분리.
- **PR 분리:** PR1 = 뷰 저장·적용·관리(REQ-001, REQ-003 — 같은 훅/컴포넌트를 공유하고 위험·리뷰 부담 축에서 갈리지 않아 함께 묶음), PR2 = 뷰 공유(REQ-002 — 외부 API 실패/재시도라는 별개 위험 축이라 분리).
...
- **PR 분리 ≠ REQ 개수**: REQ는 3개(001/002/003)지만 PR은 2개다. REQ-001·REQ-003은 같은 훅(`useSavedViews`)·같은 컴포넌트를 공유하고 위험·리뷰 부담 축에서 갈리지 않아 PR1로 묶였다. REQ-002만 외부 API 연동이라는 별개 위험 축(실패/재시도) 때문에 PR2로 분리됐다 — "REQ가 3개니까 PR도 3개"가 아니다.
