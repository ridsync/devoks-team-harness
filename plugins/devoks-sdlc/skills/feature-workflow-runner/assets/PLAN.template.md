...
## 3. Tasks
> 형식: `- [ ] TASK-ID [P?] 설명 — size: <S|M|L> — file: <경로> — traces: <REQ/AC/CTR/EDGE>`
...
### PR1 — <핵심 작업>
- [ ] `TASK-001` 공유 타입/상수 정의 — size: S — file: `<경로>` — traces: CTR-001
- [ ] `TASK-002` [P] 계산 로직 훅/유틸 + 테스트 — size: M — file: `<경로>` — traces: CTR-001, REQ-001
- [ ] `TASK-003` 화면/라우트 + 상태 처리 — size: M — file: `<경로>` — traces: REQ-001, AC-001-1
- [ ] `TASK-004` 결과/조건부 렌더링 — size: S — file: `<경로>` — traces: AC-001-2

<!-- PR을 나눌 근거(기능/위험/리뷰 부담)가 §1에 명시됐을 때만 PR2 이상을 추가한다 — 기준: `references/task-pr-splitting.md`.
### PR2 — <두 번째 큰 작업>
- [ ] `TASK-010` <...> — size: M — file: `<경로>` — traces: REQ-002
- [ ] `TASK-011` 에러/재시도 처리 — size: S — file: `<경로>` — traces: EDGE-002
-->
