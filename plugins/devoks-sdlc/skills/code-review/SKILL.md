...
- **Lint/Format**: 프로젝트의 lint/format 도구 결과를 먼저 확인한다. **lint 오류가 있는 상태로 리뷰 진행 금지.**
- **타입 검사**: 프로젝트의 타입 검사 도구가 있다면 컴파일 오류 없음을 확인한다.
...
**필수 체크리스트 (5개 항목 — 리뷰 중 하나도 빠뜨리지 않는다):**
- [ ] 잠재적 버그는 없는가?
- [ ] 예외 처리가 빠진 곳은 없는가?
- [ ] 성능상 문제는 없는가?
- [ ] 더 읽기 좋은 코드가 있는가?
...
각 항목에서 발견된 이슈는 아래 "주요 이슈" 섹션에 심각도별로 기록하고, 항목별 확인 결과는 "체크리스트 커버리지" 섹션에 남긴다.
