...
PLAN.md를 SSOT로 태스크를 의존성 순서대로 실행하고 진행을 추적한다. `devoks:feature-workflow-runner` 의 **Phase 3만** 단독 실행한다.
...
```
/devoks:plan-executor [plan=<PLAN.md 경로>] [pr=<특정 PR 그룹만>] [task=<특정 TASK-ID만>]
```

- `plan` 없으면 가장 최근 `.claude/workspace/*/PLAN.md` 를 후보로 제시·확인한다. → `../devoks-feature-workflow-runner/references/output-location.md`
- **코드는 PLAN Task의 `file:`(프로젝트 본래 위치)에 작성**한다. 워크스페이스에는 문서(진행 보고 등)만, 코드는 두지 않는다.

## 절차

1. **상태 로드** — PLAN.md를 읽어 `[x]`/`[ ]` Task와 §4 의존성 그래프를 파악. 세션 Task/Todo를 PLAN 상태에 맞춘다. → `../devoks-feature-workflow-runner/references/progress-tracking.md`
...
3. **작업 브랜치 준비 (최초 1회)** — 첫 구현 착수 **직전**에 현재 브랜치를 확인한다. `main`(원격 기본 브랜치)이면 작업 목적을 담은 의미론적 이름의 로컬 브랜치를 만들고 체크아웃한 뒤 진행한다. `main`이 아니면(이미 작업 브랜치) 새로 만들지 않고 "이 브랜치에서 진행" 여부만 한 줄로 확인 후 진행한다.
   - **타입**: `feat`·`fix`·`docs`·`style`·`refactor`·`perf`·`test`·`build`·`ci`·`chore` 중 작업 성격에 가장 가까운 것 (Conventional Commits — `devoks:git-commit-msg` 규칙).
...
- DoD 각 항목 점검 후 `[x]`: 커버리지 누락 0 재점검(`../devoks-feature-workflow-runner/references/traceability.md`), 핵심 테스트 통과 확인, Edge case 확인.
